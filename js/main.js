...
(function form() {
    ...
    f.addEventListener('submit', e => {
        ...
        if (!name || !email || !service) {
            note.textContent = 'Vyplňte prosím jméno, e-mail a vyberte službu.';
            note.style.color = 'var(--rose, #C97B7B)';
            return;
        }
        if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
            note.textContent = 'Zkontrolujte prosím e-mailovou adresu.';
            note.style.color = 'var(--rose, #C97B7B)';
            return;
        }
        // Honeypot
        if (f['bot-field'] && f['bot-field'].value) return;

        note.textContent = 'Děkujeme! Vaši poptávku jsme přijali — ozveme se nejpozději do druhého pracovního dne.';
        note.style.color = 'var(--teal, #1B3A47)';
        f.reset();
    });
})();
